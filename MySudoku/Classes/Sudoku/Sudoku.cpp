...
void Sudoku::GenerateBoard(Board * board, int level) {
    ...
    // Generate random solved board 先生成一个随机的解决方案面板
    GenerateRandomSolution(board);
    // Remove elements and push them on the stack until the board is not solvable anymore
	//删除元素并且入栈，直到面板无法解决
    do {
        possibilities = false; // 我们在上一次迭代中保持追踪任何被删除的元素 We keep track of any removed elements in the last iteration
